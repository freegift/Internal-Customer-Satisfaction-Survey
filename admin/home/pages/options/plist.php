<form name="list" action="" method="POST">
    <input type="hidden" value="list_option" name="form" />
    <div id="controls">
        <?php
...
<table id="list" class="list-table ui-widget-content ui-corner-all" >
    <thead>
        <tr class="tr ui-widget-header">
            ...
            <th class="th">S/N</th>        
            <th class="th" style="min-width:300px;">OPTIONS</th>
    ...
    <tbody>
<?php
$record = $cControllers->ListAll("options" ,"o_c_id",$_SESSION['DEFAULT']['c_id']);
//var_dump($record);
//die('ok');
if (!empty($record)){
    $i = 0;
    ...
    $b = '';//($value["e_view"] < 1) ? 'bold' : '';
        ...
        <tr class="tr <?php echo $b;?>">
            <td class="td <?php echo $bg_row;?>"><input type="checkbox" class="item" name="id[<?php echo $value["o_id"];?>]" id="id" value="<?php echo $value["o_id"];?>" /></td>
            <td class="td <?php echo $bg_row;?>"><?php echo (int)$i;?></td>
            <td class="td <?php echo $bg_row;?>"><?php echo $value["o_name"];?></td>
            <td class="td <?php echo $bg_row;?>"><?php echo $value["o_score"];?></td>
            <td class="td <?php echo $bg_row;?>"><?php echo $value["o_percent"];?></td>
            <td class="td <?php echo $bg_row;?> td-center"><?php echo $cApp->EnableStatus($value["o_enable"]);?></td>
            <td class="td <?php echo $bg_row;?> td-action">
                <?php //if ($cApp->HasPrivilege("P_USER_VIEW")) { ?>    
                    <input name="btnedit" class="view" type="button" value="Edit" onclick="link_direct('<?php echo BASE_DIR."controllers/controllers.php?p=options&id=".$value["o_id"];?>');" />
                <?php // } ?>
            </td>

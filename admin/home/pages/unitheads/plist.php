<?php // include_once 'pcopyusersmapping.php';?>
...
<table id="list" class="list-table ui-widget-content ui-corner-all" >
    ...
    <tbody>
<?php
$record = $cControllers->ListAllUnitHeadMappingRecords();
//var_dump($record);
//die('ok');
if (!empty($record)){
    $i = 0;
        ...
        <tr class="tr <?php echo $b;?>">
            ...
            <?php
                foreach ($value["ext"] as $key_ext => $value_ext) {
                    echo "<strong>* ". $value_ext["d_target"]."</strong><br>";
                }
            ?>
            </td>
            <td class="td <?php echo $bg_row;?> td-center">
                 <?php //if ($cApp->HasPrivilege("P_USER_VIEW")) { ?>    
                    <input name="btnedit" class="view" type="button" value="Edit" onclick="link_direct('<?php echo BASE_DIR."controllers/controllers.php?p=unit-heads&id=".$value["u_id"];?>#mapheads');" />
                <?php // } ?>
            </td>
